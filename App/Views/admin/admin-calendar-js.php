...
function makePageEditable(item) // called in menu Admin > Edit content
{
  if (($(".editablecontent").length != 0))
  {
      ...
      $("#oldcalendar").val($("#editablecalendarcontent").html());
...
function turnOffEditing(item)
{
  ...
  if ($('#oldcalendar').val() != '')
  {
      $(".editablecontent").html($("#oldcalendar").val());
  }
}
...
function saveEditedPage()
{
  ...
  $("#editcalendarpage").unbind('submit').ajaxSubmit(options);
