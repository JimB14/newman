...
function makePageEditable(item) // called in menu Admin > Edit content
{
  if (($(".editablecontent").length != 0))
  {
      ...
      $("#oldaddtestimonial").val($("#editableaddtestimonialcontent").html());
...
function turnOffEditing(item)
{
  ...
  if ($('#oldaddtestimonial').val() != '')
  {
      $(".editablecontent").html($("#oldaddtestimonial").val());
  }
}
...
function saveEditedPage()
{
  ...
  var addtestimonialpagedata = editor.getData();

  // store data (pagedata) into hidden input field with name & id of "thedata" -- #the data -- (~ line #21)
  $("#theaddtestimonialdata").val(addtestimonialpagedata);
  ...
  $("#editaddtestimonialpage").unbind('submit').ajaxSubmit(options);
