...
function makePageEditable(item) // called in menu Admin > Edit content
{
  if (($(".editablecontent").length != 0))
  {
      ...
      $("#oldgetquote").val($("#editablegetquotecontent").html());
...
function turnOffEditing(item)
{
  ...
  if ($('#oldgetquote').val() != '')
  {
      $(".editablecontent").html($("#oldgetquote").val());
  }
}
...
function saveEditedPage()
{
  ...
  $("#editgetquotepage").unbind('submit').ajaxSubmit(options);
