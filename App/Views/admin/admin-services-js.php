...
function makePageEditable(item) // called in menu Admin > Edit content
{
  if (($(".editablecontent").length != 0))
  {
      ...
      $("#oldservices").val($("#editableservicescontent").html());
...
function turnOffEditing(item)
{
  ...
  if ($('#oldservices').val() != '')
  {
      $(".editablecontent").html($("#oldservices").val());
  }
}
...
function saveEditedPage()
{
  ...
  $("#editservicespage").unbind('submit').ajaxSubmit(options);
